    ...
    Context "SSH Key Generation" {
        ...
        It "Should configure multiple SSH users for Azure DevOps" {
            ...
            if (Test-Path $aliasConfigPath) {
                $content = Get-Content $aliasConfigPath -Raw
                # Azure should have multiple sshCommand entries (git and username)
                $sshCommandCount = ([regex]::Matches($content, 'sshCommand')).Count
                ($sshCommandCount -ge 2) | Should Be $true
            }
